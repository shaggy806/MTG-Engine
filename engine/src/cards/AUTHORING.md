...
every kind when `counter` is omitted, "the number of counters on it" —
...
(Condemn; each takes a target slot, `"source"`, `"trigger-object"`,
`"sacrificed"` or `"tapped"` — the one permanent tapped to pay the ability's
...
`{ castThisTurn: CardFilter, who?, greatest?: "mana-value" }` (the spells `who`
— you by default — cast this turn that match, as they were cast: how many, or
the greatest mana value among them, {X} counted — Rootha, Mastering the
Moment's "the greatest mana value among instant and sorcery spells you've cast
this turn"),
...
| `lose-abilities` | `target`, `loseLandTypes?`, `activated?`, `duration` | "It loses all abilities" (layer 6) — and, with `loseLandTypes`, "all land types" (layer 4; its other types and supertypes stay) — and "has" `activated`, granted by the same effect so they aren't lost with the rest (rule 613.7). Ultima, Origin of Oblivion's blight is an `add-counter` of `blight` and then this, with `loseLandTypes`, `activated: [{T}: Add {C}]` and `duration: { whileCounter: "blight" }`. |
| `grant-activated` / `grant-activated-all` | `target` or `filter`, `ability`, `duration` | The activated-ability twins of `grant-triggered` and `grant-triggered-all`: Rain of Filth's "until end of turn, lands you control gain 'Sacrifice this land: Add {B}.'" is `grant-activated-all` over `{ type: "land", controlledBy: "you" }`. A granted mana ability pays for things like a printed one — or, like any mana ability the auto-payer can't plan with (no `{T}`, here), is activated by hand. The ongoing equivalent is `StaticAbility.grantsActivated`. Keep the ability's `resolve: null`: it lives in the game state. |
| `grant-triggered-all` | `filter`, `ability`, `duration` | The mass form of `grant-triggered`: every permanent matching `filter` gains the ability — Azlask, the Swelling Scourge's "Scions and Spawns you control gain … annihilator 1 until end of turn" (`ability: annihilator(1)`). Matches are fixed as it resolves; a token stack gains it whole. |
...
| `animate` | `target`, `power`, `toughness`, `addTypes`, `addSubtypes`, `setSubtypes?`, `setColors?`, `loseAbilities?`, `keywords?`, `duration` | man-lands, Turn to Frog. `power`/`toughness` take a live amount, read once as the effect applies and fixed (rule 608.2h): Zur, Eternal Schemer's "base power and base toughness each equal to its mana value" is `{ manaValueOf: 0 }` each. |
...
| kind | fields |
| --- | --- |
| `create-token` | `token` (a registry name), `count`, `who?: "target-controller" \| PlayerScope` (Beast Within — under `targets[0]`'s controller; a scope has each of its players create `count` — "each opponent creates a Treasure token"), `tapped?` (Army of the Damned — "create thirteen **tapped** … tokens"; a tapped batch is never folded into a token stack, since a stack carries one `tapped` flag for all of it) `gainUntilEndOfTurn?: Keyword[]` is "they gain haste until end of turn" (Ovika, Enigma Goliath): the keywords go on the tokens this makes as they're made, never on ones already there, and such a batch only folds into a token stack made the same way. `goadedForGame?: true` is "the tokens are goaded for the rest of the game" (Rendmaw, Creaking Nest), by the effect's controller — a token of their own included — made that way, as `gainUntilEndOfTurn` is. `basePt?: { power, toughness }` is "create an **X/X** token" (Rootha, Mastering the Moment): the token's base P/T, set as it's made, as part of what it is (rule 111.3), so counters and anthems go on top and a later "becomes 1/1" replaces it. The token definition prints 0/0. `thenCounters?: { kind, amount }` is "create a token, **then put N counters on it**" (Zaxara, the Exemplary's 0/0 Hydra with `{ kind: "+1/+1", amount: "x" }`): the effect's controller puts them on each token this made once it has entered — counters put on, not ones it entered with — before state-based actions see a 0/0; such tokens are always made as separate objects, so a stack of the same token already there never absorbs them and their counters. |
| `create-token-copy` | `of: "source" \| "trigger-object" \| slot`, `count`, `gainsHaste?`, `exileAtEndStep?`, `notLegendary?`, `basePt?: [p, t]`, `who?: "you"` — a token that's a copy of a permanent, under *its* controller by default; `who: "you"` puts it under the effect's controller instead, which is what a card copying something an **opponent** controls means (Hate Mirage). `"trigger-object"` = the permanent whose entering/attacking fired the trigger (Miirym); a slot = a target (Saw in Half). `gainsHaste` is the copy exception "except it has haste" (Kiki-Jiki), which lasts; `gainUntilEndOfTurn?: Keyword[]` is "it gains haste until end of turn" (Mishra, Eminent One). `exceptions?: { name?, setTypes?, setSubtypes?, addTypes?, addSubtypes?, setColors?, addColors?, basePt?, keywords?, activated? }` is the rest of "a copy of it, **except** …" (rule 707.9b): Anikthea, Hand of Erebos's "except it's a 3/3 black Zombie creature in addition to its other types" is `{ basePt: [3, 3], setColors: ["B"], addTypes: ["creature"], addSubtypes: ["Zombie"] }`; Mishra, Eminent One's "except its name is Mishra's Warform" is `name`. `setTypes` is "it's an enchantment and loses all other card types" (Myrkul, Lord of Bones), taking the subtypes that went only with the lost types (rule 205.1a — no Bear, no Food, no Forest; `subtypeFitsTypes`). `activated` is "and it has '{2}, {T}, Sacrifice this token: You gain 3 life.'" (Brenard): the copy has those activated abilities — offered, activated and paid for with like its own, a mana ability included — and loses them to a later "loses all abilities" (rule 613.7); keep their `resolve: null`, since they live in the game state. The exceptions are the copy's **copiable values**: a `copiable` modifier under every other effect (a later Turn to Frog replaces the P/T and takes the keywords), and anything that copies the copy — a token copy, a Clone — copies them too, a newer copy's own winning where they differ. `gainsHaste`, `basePt` and `notLegendary` are copy exceptions too, and carry the same way. A new name is what the legend rule and `name` filters read (`nameOf`); the registry is still keyed by the copied card. A copy of something that has since left the zone the effect refers to it in — "a copy of that creature" of one that died (Ratadrabik of Urborg), its `trigger-object`, source or a target that was a permanent — copies it **as it last existed there** (rule 608.2h): a Clone that died as a copy of a legend gives a copy of that legend, and a token that ceased to exist is still copied, exceptions and all. A card targeted where it is now (Anikthea's exiled card) is read as it is. `asCard: true` copies **the card as it is now** instead of as it last existed: Myrkul's "a copy of that card" — the card it just exiled — is a copy of Bloodline Keeper however transformed the creature died, where Brenard's "a copy of that creature" is a Lord of Lineage. A token copy is still never asked an "as this enters" choice (§15). |
...
- **`modal { minModes, maxModes, modes: ModeOption[] }`** — a choice made as
  the effect resolves (`maxModes` may be live — Riku of Many Paths' "choose up
  to X" is its trigger value): "add one mana of any color", "create a Food token or a
  ...
  and `pool.test.ts` fails a spell that doesn't. A modal *triggered* ability
  printed "Choose one —" with bullets uses this effect with **`announced:
  true`** as its whole effect: its modes are chosen as it goes on the stack,
  before its targets (rules 603.3c, 700.2b), carried on the ability
  (`chosenModes`) and applied as it resolves without asking again, and with
  none chosen it's removed from the stack. A trigger that fires for each of a
  token stack's members goes on the stack one copy at a time, each announcing
  its own. `maxModes` may be live, read as it triggers (Riku of Many Paths'
...
- **`unless { chooser, options, otherwise }`** — a *punisher* clause, where
  **someone else** decides whether to pay and `otherwise` happens only if they
  ...
  target-slot index holding a player — or an object, whose controller
  decides: Spell Pierce's "counter target noncreature spell unless **its
  controller** pays {2}" is `chooser: 0` with `otherwise: { kind: "counter",
  target: 0 }` — `"trigger-controller"` (the
...
it last was. So are `damagedThisTurnBy: { subtype?, type?, controlledBy? }` —
"dealt damage this turn by a Spider **you controlled**" (Shelob, Child of
Ungoliant), matched against each source as it was when it dealt the damage
(`GameObject.damageThisTurn`) — `excessDamageThisTurn` (rule 120.4a: more
than the lethal damage it needed then — toughness less damage already marked,
1 from a deathtouch source, none once it had lethal damage; a planeswalker's
loyalty — Maarika, Brutal Gladiator's `toFilter`) and
`dealtDamageToCreatureThisTurn` (it dealt damage to another creature). `cast` / `castBy: "you"` / `castFrom` / `enteredFrom` /
...
| `attacks` | `who`, `filter?`, `otherOnly?`, `attackingYou?`, `defender?`, `aloneAgainstDefender?`, `defenderLife?` | a creature is declared as an attacker, once per attacker (`filter` narrows which one — Utvara Hellkite / Atarka, World Render: "a Dragon you control"; `otherOnly` is Arahbo's "whenever **another** Cat you control attacks"). The attacker is the trigger object ("it gets +X/+X" — `target: "trigger-object"`) and the player it attacks, or the controller of the planeswalker it attacks, is the `"trigger-player"` ("defending player"); the player or planeswalker itself is what `damage.toTriggerRecipient` hits (Mage Slayer's "the player or planeswalker it's attacking" — a planeswalker gone by then is dealt nothing). `attackingYou` fires only when the attack is aimed at this permanent's controller (Kazuul's "if you're the defending player") — which also covers "a creature an opponent controls", since nobody can attack themselves. `defender: "player"` is "attacks **a player**" / "attacks **an opponent**" (Kaalia of the Vast): a creature attacking a planeswalker attacks the planeswalker, not its controller (rule 508.3a), so it doesn't count. `aloneAgainstDefender` is "…, **if no other creatures are attacking that player**": an intervening-if, checked against the whole declaration and again on resolution. `defenderLife` is how the attacked player's life ranks (a `DefenderLife`; a planeswalker never ranks): `"most"` is dethrone's "attacks the player with the most life or tied for most life" — every player in the game compared, you included, asked once as it triggers (use `dethrone()` for the keyword itself) — and `"more-than-another-opponent"` is "if that opponent has more life than another of your opponents", an intervening-if asked again on resolution. Every attacker is attacking before the first of these fires, so a trigger always sees the complete declaration. A creature put onto the battlefield attacking was never declared and fires none of this. |
| `attacks-player` | `who`, `defender`, `defenderLife?` | "whenever a player attacks one of your opponents" (Breena, the Demagogue: `{ who: "any", defender: "opponent" }`, and her "if that opponent has more life than another of your opponents" is `defenderLife: "more-than-another-opponent"` — see `attacks`), "whenever an opponent attacks you" (`{ who: "opponent", defender: "you" }`). Once **per player attacked**, however many creatures attack them; creatures attacking only a planeswalker attack nobody here. The attacked player is the `"trigger-player"` ("that opponent"), the attacking player is always the active player (`"active-player"` — "that attacking player draws a card"), and `{ triggerValue: true }` is how many creatures attack that player. |
...
the host as the event happened, whoever controls it, while the ability stays
the Equipment's or Aura's and its controller's ("you", "an opponent").
A host that died is still the attachment's until the next state-based check
(rule 704.5n), and an attachment destroyed alongside it remembers the host
it had (rule 603.10a), so Skullclamp draws under a wrath too. A granted
ability (Sticky Fingers' "Whenever **this** creature deals combat damage")
is the creature's own instead — `grantsTriggered` with `who: "self"`. The
`equip(cost)` helper (`cards/helpers.ts`) is the standard Equip ability.
...
determines (`becomes-target`'s targeting player) is auto-filled; a player the
event names but that isn't targeted ("that player") is the `"trigger-player"`
scope.
...
- `{ kind: "all", of: [...] }` — every one of the conditions holds. Michelangelo,
  the Heart's "at the beginning of your second main phase, if you attacked this
  turn" is a `postcombat-main` trigger whose condition is a `turn-structure`
  `mainPhase: 2` and a `turn-stat` `attacked` together.
...
- `{ event: "enters-battlefield", tapped?, tappedUnless?,
  ...
  `counters.amount` is `"x"` for the X it was cast with (rule 107.3m) or any
  live amount, read as it enters and never counting itself or what enters
  with it (Aeve, Progenitor Ooze's "a +1/+1 counter on it for each other Ooze
