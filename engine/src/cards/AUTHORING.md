...
| `create-token-copy` | `of: "source" \| "trigger-object" \| slot`, `count`, `gainsHaste?`, `exileAtEndStep?`, `notLegendary?`, `basePt?: [p, t]`, `who?: "you"` — a token that's a copy of a permanent, under *its* controller by default; `who: "you"` puts it under the effect's controller instead, which is what a card copying something an **opponent** controls means (Hate Mirage). `"trigger-object"` = the permanent whose entering/attacking fired the trigger (Miirym); a slot = a target (Saw in Half). `gainsHaste` is the copy exception "except it has haste" (Kiki-Jiki), which lasts; `gainUntilEndOfTurn?: Keyword[]` is "it gains haste until end of turn" (Mishra, Eminent One). `exceptions?: { name?, addTypes?, addSubtypes?, setColors?, addColors?, basePt?, keywords? }` is the rest of "a copy of it, **except** …" (rule 707.9b): Anikthea, Hand of Erebos's "except it's a 3/3 black Zombie creature in addition to its other types" is `{ basePt: [3, 3], setColors: ["B"], addTypes: ["creature"], addSubtypes: ["Zombie"] }`; Mishra, Eminent One's "except its name is Mishra's Warform" is `name`. The exceptions are the copy's **copiable values**: a `copiable` modifier under every other effect (a later Turn to Frog replaces the P/T and takes the keywords), and anything that copies the copy — a token copy, a Clone — copies them too, a newer copy's own winning where they differ. `gainsHaste`, `basePt` and `notLegendary` are copy exceptions too, and carry the same way. A new name is what the legend rule and `name` filters read (`nameOf`); the registry is still keyed by the copied card. A copy of something that has since left the zone the effect refers to it in — "a copy of that creature" of one that died (Ratadrabik of Urborg), its `trigger-object`, source or a target that was a permanent — copies it **as it last existed there** (rule 608.2h): a Clone that died as a copy of a legend gives a copy of that legend, and a token that ceased to exist is still copied, exceptions and all. A card targeted where it is now (Anikthea's exiled card) is read as it is. |
