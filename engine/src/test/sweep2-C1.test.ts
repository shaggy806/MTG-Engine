...
describe("Jin Sakai, Ghost of Tsushima", () => {
  it("a lone attacker gains double strike; Jin draws on each hit", () => {
    const { game, a } = setUp();
  ...
  it("the Ghost mode makes it unblockable", () => {
    const { game, a } = setUp();
    const jin = spawn(game, "Jin Sakai, Ghost of Tsushima");
    a.declareAttackersFn = () => [{ attacker: jin, defender: B }];
    a.chooseModesFn = () => [1];
    toStep(game, "declare-blockers");
    expect(game.viewFor(A).objects[jin]?.keywords).toContain("unblockable");
  });

  it("doesn't trigger when another creature attacks the same player", () => {
    const { game, a } = setUp();
    const jin = spawn(game, "Jin Sakai, Ghost of Tsushima");
    const bear = spawn(game, "Grizzly Bears");
    a.declareAttackersFn = () => [
      { attacker: jin, defender: B },
