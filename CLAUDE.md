...
hot-seat client on top of it (M6b), then follow-ups — **`order-blockers` is now
a dispatched action** too (so *every* decision is an action; `chooseTargets` for
triggered-ability targets is the last synchronous controller callback), and the
client gained Scryfall card art + a blocker-ordering UI. Implemented — turn/step
...
/ `order-blockers` / `discard` alongside the priority actions), **`legalActions(player)`**
...
- **`ui/`** — `CardTile` (Scryfall `art_crop` background via `?format=image` with a text-only fallback on load error; `highlight` = must-pick, `selected`, `activatable` = dashed marker, `badge` = ⚔/🛡, `order` = damage-order ordinal), `PhaseTrack`, `PlayerPanel` (life/mana/zone counts; `targetable` for player targets), `Stack`, `EventLog` (auto-scroll).
