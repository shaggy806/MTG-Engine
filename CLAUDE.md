...
- **`ui/`** — `CardTile` (Scryfall `art_crop` background via `?format=image` with a text-only fallback on load error; `highlight` = must-pick, `selected`, `activatable` = dashed marker, `badge` = ⚔/🛡, `order` = damage-order ordinal; `KEYWORD_ABBR` uses the engine's hyphenated keyword spellings), `PhaseTrack`, `PlayerPanel` (life/mana/zone counts; `targetable` for player targets), `Stack`, `EventLog` (auto-scroll).
