...
- **`App.tsx`** — `App` (topbar, `PhaseTrack`, seat banner, "pass the device" curtain re-armed on seat change) wraps `<Table key={game.revision}>` — keying on revision resets all in-progress selection when the game moves on (no reset effect needed). `Table` classifies `actions` into maps (land/cast by card, abilities by source) and a `mode` (`discard` → `order-blockers` → `attackers` → `blockers` → `targeting` → `priority`); click handlers build the matching `Action`. Targeting: click highlighted `targetOptions[slot]` (a `CardTile` or a `PlayerPanel`); multi-slot picks accumulate then dispatch. Attackers/blockers/discard/order-blockers: select-then-confirm (order-blockers = click the blockers in damage order, or "keep default").
- **`ui/`** — `CardTile` renders a full card face: title bar (name + mana cost as coloured pips parsed from the `{…}` string), Scryfall `art_crop` image (colour-tinted fallback on load error), a type line, and a text box (keywords in bold + rules text, minus text that only restates the keywords); P/T overlays the art corner. Props: `highlight` = must-pick, `selected`, `activatable` = dashed marker, `badge` = ⚔/🛡, `order` = damage-order ordinal. Tiles are 138×190; tapped ones rotate 90° + scale to stay in their slot. Also `PhaseTrack`, `PlayerPanel` (life/mana/zone counts; `targetable` for player targets), `Stack`, `EventLog` (auto-scroll).
