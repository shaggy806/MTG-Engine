...
- **`game/useGame.ts`** — owns the one `Game` (in `useState`, lazy-init; `revision` counter forces re-render). `?seed=N` in the URL overrides the starting seed (repro aid); "New game" reseeds randomly. `dispatch(action)` → `game.dispatch` then `settleAndAutoPass`: `advanceUntil` a human decision (game over / `awaiting` / a priority holder), then auto-pass any "dead window" — the holder's only legal action is `pass-priority` — so a player is never asked to click "Pass" when there's nothing else to do, including on their own main phase. `passTurn()` (the "Pass Turn" button) is a deliberate, coarser fast-forward: `passRestOfTurn` dispatches `pass-priority` for whoever currently holds it — hot-seat, so it's the same person either way — through the rest of the *current* turn, ignoring whether the holder actually has some other legal action, stopping only at that turn's `end` step, a real decision (`awaiting` — attackers/blockers/discard/order-blockers still need answering), or game over; it never reads past the turn it started in. `seat` = `awaiting.player ?? priority.holder ?? activePlayer`. Exports `view` (`game.viewFor(seat, {revealAll})`), `actions` (`legalActions(seat)`), `nameOf`, `reset`.
