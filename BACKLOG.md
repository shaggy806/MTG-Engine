...
**295 of the 500 most-played commanders are implemented** (`top-commanders.txt`; re-mark with
...
- **Build down the greedy order.** `npm run cmdrs:gaps -w engine` ranks every missing engine
  ...
  `effect:duration-extensions` (+1), `effect:modifier-grants-activated` (+1),
  `keyword:toxic` (+1), `cost:mana-spending-rules` (+3).
