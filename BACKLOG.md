...
- **Ready to author, no engine work** (each needs `card:lookup` first, so they wait for a
  ...
  Calamity (`add-types`); Zoraline, Cosmos Caller (`may`'s `costLife`).
- **Build down the greedy order.** `npm run cmdrs:gaps -w engine` ranks every missing engine
  ...
  `trigger:combat-trigger-extensions` (+3), `keyword:earthbend` (+1), `trigger:sacrifice-filter`
  (+1), `condition:filter-dynamic-compare` (+1), `effect:amount-new-variants` (+1),
